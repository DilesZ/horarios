...
describe("Cobertura presencial 17:00-18:00", () => {
  ...
  test("al añadir libre a Kike el 03/07 mantiene 18h en oficina el 29 y 30 de junio y 17h el viernes", () => {
    const { generateSchedule, EMPLOYEES, DEFAULT_VACATION_PLAN_2026 } = loadSchedulingCore("src/app.jsx");
    const vacationPlan = JSON.parse(JSON.stringify(DEFAULT_VACATION_PLAN_2026));
    vacationPlan.Kike = [...(vacationPlan.Kike || []), "2026-07-03"].sort();
    const { schedule, days } = generateSchedule(2026, vacationPlan);

    const hasOfficeCoverageByType = (dayId, typeKey) => {
      const day = days.find((entry) => entry.id === dayId);
      return EMPLOYEES.some((emp) => {
        return schedule[emp.id][dayId] === typeKey && isInOffice(emp, day.weekdayLetter);
      });
    };

    expect(hasOfficeCoverageByType("2026-06-29", "O42")).toBe(true);
    expect(hasOfficeCoverageByType("2026-06-30", "O42")).toBe(true);
    expect(hasOfficeCoverageByType("2026-07-03", "O40")).toBe(true);
  });
});
...
describe("Registro formal de equidad distributiva", () => {
  ...
  test("todos los empleados tienen al menos 6 semanas intensivas", () => {
    const { generateSchedule, EMPLOYEES, DEFAULT_VACATION_PLAN_2026 } = loadSchedulingCore("src/app.jsx");
    const { schedule, days } = generateSchedule(2026, DEFAULT_VACATION_PLAN_2026);
    const weeksMap = {};
    days.forEach((day) => { weeksMap[day.weekIndex] = weeksMap[day.weekIndex] || []; weeksMap[day.weekIndex].push(day); });
    const intensiveByEmp = {};
    EMPLOYEES.forEach((emp) => {
      intensiveByEmp[emp.name] = 0;
      Object.keys(weeksMap).forEach((wi) => {
        if (weeksMap[wi].every((day) => schedule[emp.id][day.id] === "O30")) intensiveByEmp[emp.name]++;
      });
    });
    EMPLOYEES.forEach((emp) => {
      expect(intensiveByEmp[emp.name]).toBeGreaterThanOrEqual(6);
    });
  });
