  ...
  if (year === 2026) {
    ...
    if (ariel && luis) {
      ...
      if ((currentCounts[ariel.id] || 0) < 6) {
        ...
        const candidateWeeks = Object.keys(weeksLocal)
          .map(Number).sort((a,b) => a-b)
          .filter(wi => !vacWeeksByEmp[ariel.id].has(wi) && weeksLocal[wi].every(d => d.id >= "2026-06-15" && d.id <= "2026-09-18"));
        ...
        for (const wi of candidateWeeks) {
          ...
          weekDays.forEach(d => { strictAudit.schedule[ariel.id][d.id] = "O30"; });
    ...
    function ensureMinSixWeeksAllFinal(sched) {
      ...
      const countW = (s, id) => {
        let c = 0;
        Object.keys(weeksL).forEach(wi => { if (weeksL[wi].every(d => s[id][d.id] === "O30")) c++; });
        return c;
      };
      let improved = true;
      while (improved) {
        ...
        for (const emp of under) {
          ...
          for (const wi of wIdxs) {
            ...
            if (ok) {
              const n = enforceStrictWeeklyRules({ employees: EMPLOYEES, days, schedule: sched, getLateGroupForWeek });
              if (countW(n.schedule, emp.id) > counts[emp.id]) {
                EMPLOYEES.forEach(e => { days.forEach(day => { sched[e.id][day.id] = n.schedule[e.id][day.id]; }); });
                improved = true; break;
              }
            }
            EMPLOYEES.forEach(e => { days.forEach(day => { sched[e.id][day.id] = snap[e.id][day.id]; }); });
          }
          if (improved) break;
        }
      }
