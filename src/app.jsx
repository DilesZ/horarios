...
const generateSchedule = (year, vacationPlan) => {
  ...
  if (year === 2026) {
    ...
    const evaluateWeekOfficeCoverage_Local = (sched, weekDays) => {
      ...
      const missingOfficeO42Days = weekDays
        .filter((day) => day.weekdayLetter !== "V")
        .filter((day) => {
        ...
        .map((day) => day.id);
      const hasFridayOfficeO40 = !friday
        ? true
        : EMPLOYEES.some((emp) => {
      ...
      while (improved && safety < 30) {
        ...
        for (const emp of under) {
          ...
          for (const wi of candidates) {
            if (improved) break;
            const wDays = weeksL[wi];
            wDays.forEach(d => { if (sched[emp.id][d.id] !== "V") sched[emp.id][d.id] = "O30"; });
            improved = true; break;
          }
        }
      }
    ...
    function repairConflictingWeekCoverage(sched) {
      ...
      Object.keys(weeksMap).forEach(wiStr => {
        ...
        const candidates = EMPLOYEES.filter((emp) =>
          weekDays.some((day) => sched[emp.id][day.id] && sched[emp.id][day.id] !== "V")
        );
        const currentModes = {};
        ...
        candidates.forEach((emp) => {
          ["O40", "O42"].forEach((targetType) => {
            ...
            combos.push([{ empId: emp.id, targetType }]);
          });
        });
          ...
          for (let j = i + 1; j < candidates.length; j++) {
            ["O40", "O42"].forEach((firstType) => {
              ["O42", "O40"].forEach((secondType) => {
                combos.push([
                  { empId: candidates[i].id, targetType: firstType },
                  { empId: candidates[j].id, targetType: secondType },
                ]);
              });
            });
          }
        ...
        combos.forEach((combo) => {
          ...
          const snap = {};
          touched.forEach((id) => { snap[id] = {}; weekDays.forEach((d) => { snap[id][d.id] = sched[id][d.id]; }); });
          combo.forEach(({ empId, targetType }) => {
            weekDays.forEach((d) => { if (sched[empId][d.id] !== "V") sched[empId][d.id] = targetType; });
          });
          ...
          if (cov.ok && limitOk) {
            let wouldDrop = false;
            touched.forEach((id) => {
              const p = baselineWeeks[id], n = countWeeksByEmp_Local(sched, id);
              if (p < 6 && n < p) wouldDrop = true;
            });
            if (!wouldDrop) {
              let lostW = 0;
              touched.forEach((id) => { const n = countWeeksByEmp_Local(sched, id); lostW += Math.max(0, baselineWeeks[id] - n); });
              const score = lostW * 10000 + combo.length * 100;
              if (score < bestScore) { bestScore = score; bestCombo = combo.map((item) => ({ ...item })); }
            }
          }
          touched.forEach((id) => { weekDays.forEach((d) => { sched[id][d.id] = snap[id][d.id]; }); });
        });

        if (bestCombo) {
          bestCombo.forEach(({ empId, targetType }) => {
            weekDays.forEach((d) => { if (sched[empId][d.id] !== "V") sched[empId][d.id] = targetType; });
          });
        }
      });
      ...
      <header className="mb-8 border-b border-gray-100 pb-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            <div className="bg-brand-blue/5 p-3 rounded-2xl">
              <img src="logo.png" alt="Logo" className="h-12 w-auto object-contain" />
            </div>
            <div>
              <h1 className="text-2xl font-black text-gray-900 tracking-tight leading-none mb-1">Horarios Sistemas</h1>
              <p className="text-gray-500 text-sm font-medium">Gestión Operativa 2026</p>
            </div>
          </div>
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-12 gap-4">
          {/* Grupo Filtros */}
          <div className="lg:col-span-4 bg-gray-50/50 border border-gray-100 rounded-2xl p-2 flex gap-2 shadow-sm">
            <select
          ...
          <div className="lg:col-span-4 bg-gray-50/50 border border-gray-100 rounded-2xl p-2 flex gap-1 shadow-sm">
            {[
              ...
              <button
                ...
                className={`flex-1 rounded-xl px-2 py-2.5 text-sm font-black transition-all ${
                  viewMode === option.key
