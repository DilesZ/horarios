...
const COVERAGE_VISUALIZATION_OPTIONS = [{
  value: "stacked",
  label: "Barras superpuestas"
}, {
  value: "timeline",
  label: "Líneas temporales"
}, {
  value: "heatmap",
  label: "Heatmap"
}, {
  ...
  value: "gantt",
  label: "Gantt simplificado"
}];
const DEFAULT_VACATION_PLAN_2026 = {
  Kike: ["2026-06-22", "2026-06-23", "2026-06-26", "2026-06-29", "2026-06-30", "2026-07-03", "2026-08-17", "2026-08-18", "2026-08-19", "2026-08-20", "2026-08-21", "2026-09-18"],
  Jose: ["2026-08-03", "2026-08-04", "2026-08-05", "2026-08-06", "2026-08-07"],
  Enrique: ["2026-08-10", "2026-08-11", "2026-08-12", "2026-08-13", "2026-08-14", "2026-08-17", "2026-08-18", "2026-08-19", "2026-08-20", "2026-08-21", "2026-08-24", "2026-08-25", "2026-08-26", "2026-08-27", "2026-08-28"],
  David: ["2026-06-25", "2026-06-26", "2026-06-29", "2026-06-30", "2026-07-01", "2026-07-02", "2026-07-03", "2026-07-06", "2026-07-07", "2026-07-08", "2026-07-09", "2026-07-10"],
  Luis: ["2026-08-31", "2026-09-01", "2026-09-02", "2026-09-03", "2026-09-04", "2026-09-07", "2026-09-08", "2026-09-09", "2026-09-10", "2026-09-11", "2026-09-14", "2026-09-15", "2026-09-16", "2026-09-17", "2026-09-18"],
  Ariel: ["2026-07-14", "2026-07-15", "2026-07-16", "2026-07-17", "2026-07-20", "2026-07-21", "2026-07-22", "2026-07-23", "2026-07-24", "2026-07-27", "2026-07-28", "2026-07-29", "2026-07-30", "2026-07-31", "2026-08-25"]
};
...
const STRICT_WEEKLY_RULE_MESSAGES = {
  [STRICT_WEEKLY_RULES.INTENSIVE_FULL_WEEK_ONLY]: "No se permite jornada intensiva en días sueltos. Debe asignarse la semana operativa completa.",
  [STRICT_WEEKLY_RULES.MIXED_40_42_IN_WEEK]: "No se permite mezclar turnos de 40h y 42h dentro de la misma semana.",
  [STRICT_WEEKLY_RULES.MIXED_SHIFT_TYPES_IN_WEEK]: "La semana contiene combinación de tipos de jornada no permitida."
};
...
const getShiftGroupMemberNames = shiftGroup => EMPLOYEES.filter(employee => employee.group === shiftGroup).map(employee => employee.name);
...
const isEmployeeInOfficeForDay = ({
  employee,
  day,
  typeKey,
  forcedOfficeSet
...
const buildCoverageDayEntry = ({
  ...
  employees.forEach(employee => {
    ...
    const isInOffice = isEmployeeInOfficeForDay({
      employee,
      day,
      typeKey,
      forcedOfficeSet
...
const buildWeeklyCoverageViewModel = ({
  ...
  return Object.keys(weeksMap).map(weekIndexValue => parseInt(weekIndexValue, 10)).sort((left, right) => left - right).map(weekIndex => {
    const weekDays = weeksMap[weekIndex];
    const entries = weekDays.map(day => buildCoverageDayEntry({
      day,
      employees,
      schedule,
      forcedOfficeSet,
...
const detectStrictWeeklyViolations = ({
  employees,
  days,
  schedule
}) => {
  ...
  Object.keys(weeksMap).forEach(weekIndexRaw => {
    ...
    employees.forEach(employee => {
      ...
      if (hasAnyO30 && (!allWeekIsO30 || hasVacation)) {
        violations.push(buildWeeklyViolation({
...
const validateStrictWeeklyRules = ({
  employees,
  days,
  schedule
}) => {
  const violations = detectStrictWeeklyViolations({
    employees,
    days,
    schedule
  });
...
const getEmployeeRegularWeekType = ({
  schedule,
  employeeId,
  weekDays
}) => {
  const nonVacationTypes = weekDays.map(day => schedule[employeeId][day.id]).filter(type => type && type !== "V");
...
const enforceStrictWeeklyRules = ({
  employees,
  days,
  schedule,
  getLateGroupForWeek
  ...
  Object.keys(weeksMap).forEach(weekIndexRaw => {
    ...
    employees.forEach(employee => {
      ...
      const allFullWeekO30 = uniqueNonVacation.length === 1 && uniqueNonVacation[0] === "O30" && nonVacation.length === weekDays.length;
      ...
      if (changed) {
        correctionLog.push({
          ...
          targetType,
          before,
          message: "Se normalizó la semana para cumplir integridad estricta (sin mezcla de jornadas y sin intensiva parcial)."
        });
      }
    });
  });
...
const buildEquityAudit = ({
  ...
  const orderedWeekIndexes = Object.keys(weeksMap).map(wi => parseInt(wi, 10)).sort((a, b) => a - b);
...
const generateSchedule = (year, vacationPlan) => {
      ...
      EMPLOYEES.forEach(e => {
        ...
        if (day.weekdayLetter !== "V") {
          ...
          if (isInOffice && hasFullSchedule) {
            ...
            if (GROUP2.includes(e.name)) {
              group2HasOffice = true;
              group2Covering++;
            }
          }
          if (isInOffice && type === "O42") {
            hasO42InOffice = true;
          }
        } else {
          // El viernes solo O40 cubre hasta las 17:00; O42 sale a las 14:00.
          const mockType = type || "O40";
          if (isInOffice && mockType === "O40") {
  ...
  const MAX_FORCED_OFFICE_DAYS = 50;
    ...
    for (const day of allDays) {
      ...
      EMPLOYEES.forEach(e => {
        ...
        if (day.weekdayLetter !== 'V') {
          ...
          if (type === 'O40') {
            if (inOffice) hasO40InOfficeOnFriday = true;else candidateFriday++;
          }
        }
      });
      if (day.weekdayLetter !== 'V') {
        if (!group1HasOffice && group2Covering === 0) {
          if (candidateG1 > 0 || candidateG2 > 0) count++;else return Infinity; // Unfixable
        } else if (!group2HasOffice && group1Covering === 0) {
          if (candidateG2 > 0 || candidateG1 > 0) count++;else return Infinity; // Unfixable
        }
      } else {
        if (!hasO40InOfficeOnFriday) {
          if (candidateFriday > 0) count++;else return Infinity; // Unfixable
        }
      }
    }
    return count;
  ...
  Object.keys(weeks).forEach(wiStr => {
    ...
    const canCoverLJ = groupName => {
      const groupMembers = EMPLOYEES.filter(e => e.group === groupName);
      return weekDays.every(day => {
        if (day.weekdayLetter === "V") return true;
        return groupMembers.some(emp => {
          // Si está de vacaciones ese día concreto, no cuenta
          const vacs = vacationPlan[emp.name] || [];
          if (vacs.includes(day.id)) return false;
          const officeDays = emp.officeDays.split(",").map(d => d.trim());
          return officeDays.includes(day.weekdayLetter);
        });
      });
    };
    const aCanCover = canCoverLJ("A");
    const bCanCover = canCoverLJ("B");
    if (scoreA > scoreB && bCanCover) {
      // A tiene más presencia en viernes -> A hace O40 -> B hace O42
      weekAssignments[wi] = "B_LATE";
    } else if (scoreB > scoreA && aCanCover) {
      // B tiene más presencia en viernes -> B hace O40 -> A hace O42
      weekAssignments[wi] = "A_LATE";
    } else if (aCanCover && !bCanCover) {
      weekAssignments[wi] = "A_LATE";
    } else if (bCanCover && !aCanCover) {
      weekAssignments[wi] = "B_LATE";
    } else {
      // Ambos pueden (o ninguno puede) -> Rotación estándar basada en SHIFT_BASE_A_18H y paridad
      const defaultIsALate = SHIFT_BASE_A_18H ? wi % 2 === 0 : wi % 2 !== 0;
      if (defaultIsALate && aCanCover) {
        weekAssignments[wi] = "A_LATE";
      } else if (!defaultIsALate && bCanCover) {
        weekAssignments[wi] = "B_LATE";
      } else {
        weekAssignments[wi] = aCanCover ? "A_LATE" : bCanCover ? "B_LATE" : defaultIsALate ? "A_LATE" : "B_LATE";
      }
    }
  });
  ...
  Object.keys(weeks).forEach(wiStr => {
    ...
    const allEligible = EMPLOYEES.filter(emp => !vacWeeksByEmp[emp.id].has(wi) && (!anchor || emp.id !== anchor.id)).sort((a, b) => {
      const aP = a.name === "Enrique" || a.name === "Ariel" ? 0 : 1;
      const bP = b.name === "Enrique" || b.name === "Ariel" ? 0 : 1;
    ...
    if (nonLateHasVacation) {
      nonLate.filter(emp => !vacWeeksByEmp[emp.id].has(wi)).sort((a, b) => {
        const diff = intensiveWeeksByEmp[b.id] - intensiveWeeksByEmp[a.id];
        return diff !== 0 ? diff : a.id - b.id;
      });
    }
    ...
    weekDays.forEach(day => {
      ...
      if (!hasO40InOffice) {
        const candidates = EMPLOYEES.filter(emp => schedule[emp.id][day.id] !== "V").sort((a, b) => {
  ...
  if (year === 2026) {
    ...
    if (enrique) {
      ...
      if (schedule[enrique.id][july24] === "O40") {
    ...
    if (kike) {
      ...
      criticalDays.forEach(dayId => {
        ...
        if (dayObj && schedule[kike.id][dayId] !== "V") {
  ...
  if (year === 2026) {
    ...
    if (enrique) {
      ...
      Object.keys(weeksList).forEach(wiStr => {
        ...
        const allO30 = daysInWeek.every(day => schedule[enrique.id][day.id] === "O30");
        if (allO30) {
          intensiveCount += 1;
    ...
    while (balancing && loops < 150) {
      ...
      const sortedEmps = [...EMPLOYEES].sort((a, b) => finalIntensiveWeeks[a.id] - finalIntensiveWeeks[b.id]);
      for (const emp of sortedEmps) {
        if (finalIntensiveWeeks[emp.id] >= targetIntensiveWeeks) continue;
        const weeksIndices = Object.keys(finalWeeksMap).map(k => parseInt(k, 10)).sort((a, b) => a - b);
        for (const wi of weeksIndices) {
          ...
          if (canAssignStrict) {
            ...
            const validDonors = occupants.filter(donor => finalIntensiveWeeks[donor.id] > targetIntensiveWeeks || finalIntensiveWeeks[donor.id] > finalIntensiveWeeks[emp.id] + 1).sort((a, b) => finalIntensiveWeeks[b.id] - finalIntensiveWeeks[a.id]); // Donors with most weeks first
    ...
    const guaranteeMinimumIntensiveWeeks = minimumTarget => {
      ...
      while (changed && safety < 200) {
        changed = false;
        safety += 1;
        const underTarget = [...EMPLOYEES].filter(emp => finalIntensiveWeeks[emp.id] < minimumTarget).sort((a, b) => finalIntensiveWeeks[a.id] - finalIntensiveWeeks[b.id]);
        ...
        for (const emp of underTarget) {
          ...
          for (const wi of weeksIndices) {
            ...
            const occupants = getOccupants(wi);
            const donors = occupants.filter(donor => donor.id !== emp.id && finalIntensiveWeeks[donor.id] > minimumTarget).sort((a, b) => finalIntensiveWeeks[b.id] - finalIntensiveWeeks[a.id]);
            if (donors.length > 0) {
              const donor = donors[0];
              const snapshotDonor = {};
              const snapshotEmp = {};
              daysInWeek.forEach(day => {
                snapshotDonor[day.id] = schedule[donor.id][day.id];
                snapshotEmp[day.id] = schedule[emp.id][day.id];
              ...
              if (isValidCoverage(daysInWeek, schedule)) {
                ...
                // Revert
                daysInWeek.forEach(day => {
                  schedule[donor.id][day.id] = snapshotDonor[day.id];
                  schedule[emp.id][day.id] = snapshotEmp[day.id];
                });
              }
            }
            ...
            const snapshotEmpOnly = {};
            daysInWeek.forEach(day => {
              snapshotEmpOnly[day.id] = schedule[emp.id][day.id];
              if (schedule[emp.id][day.id] !== "V") schedule[emp.id][day.id] = "O30";
            });
            if (isValidCoverage(daysInWeek, schedule)) {
              finalIntensiveWeeks[emp.id] += 1;
              ...
              // Revert
              daysInWeek.forEach(day => {
                schedule[emp.id][day.id] = snapshotEmpOnly[day.id];
              });
            }
          }
        }
    ...
    days.forEach(day => {
      ...
      if (!hasO42) {
        ...
        const lateGroupMembers = getShiftGroupMemberNames(lateGroup);
        const candidates = EMPLOYEES.filter(emp => lateGroupMembers.includes(emp.name) && schedule[emp.id][day.id] !== "V").sort((a, b) => {
        ...
        if (pick && schedule[pick.id][day.id] === "O30" && finalIntensiveWeeks[pick.id] <= targetIntensiveWeeks) {
          const fallback = EMPLOYEES.filter(emp => schedule[emp.id][day.id] === "O40" && schedule[emp.id][day.id] !== "V").sort((a, b) => finalIntensiveWeeks[b.id] - finalIntensiveWeeks[a.id])[0];
      ...
      while (moved && loops < 120) {
        ...
        for (const emp of pending) {
          ...
          for (const wi of weekIndexes) {
            ...
            const snapshot = {};
            weekDays.forEach(d => {
              snapshot[d.id] = schedule[emp.id][d.id];
              if (schedule[emp.id][d.id] !== "V") schedule[emp.id][d.id] = "O30";
            });
            const canApply = isValidCoverage(weekDays, schedule);
            weekDays.forEach(d => {
              schedule[emp.id][d.id] = snapshot[d.id];
            });
      ...
      while ((finalIntensiveWeeks[emp.id] || 0) < targetWeeks && guard < 40) {
        ...
        for (const wi of weekIndexes) {
          ...
          const canApply = weekDays.every(day => {
            const o30Count = EMPLOYEES.filter(e => schedule[e.id][day.id] === "O30").length;
            return o30Count <= 4;
          }) && isValidCoverage(weekDays, schedule);
          if (!canApply) {
            weekDays.forEach(day => {
    ...
    improveUnderTargetWithWeekSwaps(6, ["Enrique", "Luis", "Ariel"]);
    boostEmployeeIntensiveWeeks("Luis", 6);
    boostEmployeeIntensiveWeeks("Ariel", 6);
    recalcIntensiveWeeks();
  }
  const finalStrictAudit = enforceStrictWeeklyRules({
    employees: EMPLOYEES,
    days,
    schedule,
    getLateGroupForWeek
  ...
  if (year === 2026) {
    const weeksMap = buildWeeksMap(days);
    const countWeeksByEmp_Local = (currentSchedule, empId) => {
      let count = 0;
    ...
    const countWeeksMap_Local = currentSchedule => {
      const byEmp = {};
      EMPLOYEES.forEach(emp => {
        byEmp[emp.id] = countWeeksByEmp_Local(currentSchedule, emp.id);
      });
      return byEmp;
    };
    const evaluateWeekOfficeCoverage_Local = (sched, weekDays) => {
      const friday = weekDays.find(day => day.weekdayLetter === "V");
      const missingOfficeO42Days = weekDays.filter(day => day.weekdayLetter !== "V").filter(day => {
        return !EMPLOYEES.some(emp => {
          if (sched[emp.id][day.id] !== "O42") return false;
          const officeDays = emp.officeDays.split(",").map(value => value.trim());
          return officeDays.includes(day.weekdayLetter);
        });
      }).map(day => day.id);
      const hasFridayOfficeO40 = !friday ? true : EMPLOYEES.some(emp => {
        if (sched[emp.id][friday.id] !== "O40") return false;
        const officeDays = emp.officeDays.split(",").map(value => value.trim());
        return officeDays.includes("V");
      });
      return {
        ok: hasFridayOfficeO40 && missingOfficeO42Days.length === 0,
        hasFridayOfficeO40,
        missingOfficeO42Days
      };
    };
    const getWeekModeForEmployee_Local = (sched, emp, weekDays) => {
      const nonVacationTypes = weekDays.map(day => sched[emp.id][day.id]).filter(type => type && type !== "V");
      if (nonVacationTypes.length === 0) return null;
      const uniqueTypes = [...new Set(nonVacationTypes)];
      if (uniqueTypes.length === 1) return uniqueTypes[0];
      if (uniqueTypes.includes("O42")) return "O42";
      if (uniqueTypes.includes("O40")) return "O40";
      return uniqueTypes[0];
    };
    function applyUniversalHardening(sched) {
      const weeksMap = buildWeeksMap(days);
      Object.keys(weeksMap).forEach(wiStr => {
        const weekDays = weeksMap[wiStr];
        ...
        monThu.forEach(day => {
          ...
          if (!hasOfficeO42) {
            ...
            if (pick) {
              weekDays.forEach(wd => {
                if (sched[pick.id][wd.id] !== "V" && sched[pick.id][wd.id] !== "O30") sched[pick.id][wd.id] = "O42";
              });
            }
          }
        });
    ...
    function ensureMinSixWeeksAllFinal(sched) {
      ...
      const wIdxs = Object.keys(weeksL).map(Number).sort((a, b) => a - b);
      let improved = true;
      let safety = 0;
      while (improved && safety < 30) {
        improved = false;
        safety++;
        const counts = countWeeksMap_Local(sched);
        const under = EMPLOYEES.filter(e => counts[e.id] < 6).sort((a, b) => counts[a.id] - counts[b.id]);
        if (under.length === 0) break;
        for (const emp of under) {
          if (improved) break;
          // Buscar semanas disponibles: ventana intensiva, sin vacaciones ni O30 ya asignado
          const candidates = wIdxs.filter(wi => {
            const wDays = weeksL[wi];
            if (!wDays.every(d => isIntensivePeriod(d.id))) return false;
            if (vacWeeksByEmp[emp.id].has(wi)) return false;
            if (wDays.some(d => sched[emp.id][d.id] === "V")) return false;
            if (wDays.every(d => sched[emp.id][d.id] === "O30")) return false;
            if (wDays.some(d => EMPLOYEES.filter(e => sched[e.id][d.id] === "O30").length >= 3)) return false;
            return true;
          });
          if (candidates.length === 0 && counts[emp.id] < 6) {
            // Swap: cuando emp tiene O42, buscar sustituto con O40 del mismo grupo
            for (const wi of wIdxs) {
              if (improved) break;
              const wDays = weeksL[wi];
              if (!wDays.every(d => isIntensivePeriod(d.id))) continue;
              if (vacWeeksByEmp[emp.id].has(wi)) continue;
              if (wDays.some(d => sched[emp.id][d.id] === "V")) continue;
              if (wDays.every(d => sched[emp.id][d.id] === "O30")) continue;
              if (!wDays.some(d => sched[emp.id][d.id] === "O42")) continue;
              if (wDays.some(d => EMPLOYEES.filter(e => sched[e.id][d.id] === "O30").length >= 3)) continue;
              const lg = getLateGroupForWeek(wi);
              const lgMem = getShiftGroupMemberNames(lg);
              for (const sub of EMPLOYEES) {
                if (improved) break;
                if (sub.id === emp.id) continue;
                if (vacWeeksByEmp[sub.id].has(wi)) continue;
                if (!lgMem.includes(sub.name)) continue;
                if (!wDays.some(d => sched[sub.id][d.id] === "O40")) continue;
                wDays.forEach(d => {
                  if (sched[emp.id][d.id] !== "V") sched[emp.id][d.id] = "O30";
                  if (sched[sub.id][d.id] !== "V") sched[sub.id][d.id] = "O42";
                });
                improved = true;
                break;
              }
    ...
    function repairConflictingWeekCoverage(sched) {
      const weeksMap = buildWeeksMap(days);
      Object.keys(weeksMap).forEach(wiStr => {
        const weekDays = weeksMap[wiStr];
        const initialCoverage = evaluateWeekOfficeCoverage_Local(sched, weekDays);
        if (initialCoverage.ok) return;
        const candidates = EMPLOYEES.filter(emp => weekDays.some(day => sched[emp.id][day.id] && sched[emp.id][day.id] !== "V"));
        const currentModes = {};
        candidates.forEach(emp => {
          currentModes[emp.id] = getWeekModeForEmployee_Local(sched, emp, weekDays);
        });
        const baselineWeeks = {};
        EMPLOYEES.forEach(emp => {
          baselineWeeks[emp.id] = countWeeksByEmp_Local(sched, emp.id);
        });
        const combos = [];
        candidates.forEach(emp => {
          ["O40", "O42"].forEach(targetType => {
            if (currentModes[emp.id] === targetType) return;
            combos.push([{
              empId: emp.id,
              targetType
            }]);
        ...
        for (let i = 0; i < candidates.length; i++) {
          for (let j = i + 1; j < candidates.length; j++) {
            ["O40", "O42"].forEach(firstType => {
              ["O42", "O40"].forEach(secondType => {
                combos.push([{
                  empId: candidates[i].id,
                  targetType: firstType
                }, {
                  empId: candidates[j].id,
        ...
        combos.forEach(combo => {
          const touched = [...new Set(combo.map(item => item.empId))];
          const snap = {};
          touched.forEach(id => {
            snap[id] = {};
            weekDays.forEach(d => {
              snap[id][d.id] = sched[id][d.id];
            });
          });
          combo.forEach(({
            empId,
            ...
            weekDays.forEach(d => {
              if (sched[empId][d.id] !== "V") sched[empId][d.id] = targetType;
            });
          });
          const cov = evaluateWeekOfficeCoverage_Local(sched, weekDays);
          const limitOk = weekDays.every(d => EMPLOYEES.filter(e => sched[e.id][d.id] === "O30").length <= 3);
          if (cov.ok && limitOk) {
            let wouldDrop = false;
            touched.forEach(id => {
              const p = baselineWeeks[id],
                n = countWeeksByEmp_Local(sched, id);
              if (p < 6 && n < p) wouldDrop = true;
            });
            if (!wouldDrop) {
              let lostW = 0;
              touched.forEach(id => {
                const n = countWeeksByEmp_Local(sched, id);
                lostW += Math.max(0, baselineWeeks[id] - n);
              });
              const score = lostW * 10000 + combo.length * 100;
              if (score < bestScore) {
                bestScore = score;
                bestCombo = combo.map(item => ({
                  ...item
                }));
              }
            }
    ...
    const finalizeReconciliation = sched => {
      let iters = 0,
        last = "";
      while (iters < 6) {
        ...
        repairConflictingWeekCoverage(sched);
        ...
        for (const day of wDays.filter(d => d.weekdayLetter !== "V")) {
          ...
          for (const allowO30 of [false, true]) {
            const candidates = EMPLOYEES.filter(e => {
              ...
              if (!allowO30 && sched[e.id][day.id] === "O30") return false;
              return true;
            ...
            if (pick) {
              ...
              wDays.forEach(d2 => {
                if (sched[pick.id][d2.id] !== "V") sched[pick.id][d2.id] = "O42";
              });
              if (hadO30) {
        ...
        if (fridayDay && !EMPLOYEES.some(e => sched[e.id][fridayDay.id] === "O40" && e.officeDays.includes("V"))) {
          for (const allowO30 of [false, true]) {
            const candidates = EMPLOYEES.filter(e => {
              ...
              if (!allowO30 && sched[e.id][fridayDay.id] === "O30") return false;
              return true;
            }).sort((a, b) => countWeeksByEmp_Local(sched, b.id) - countWeeksByEmp_Local(sched, a.id));
            const pick = candidates[0];
            if (pick) {
              ...
              if (hadO30) {
                wDays.forEach(d2 => {
                  if (sched[pick.id][d2.id] !== "V") sched[pick.id][d2.id] = "O40";
                });
                lostRep[pick.id + '_' + wiStr] = true;
                fixedWeeksSet.add(parseInt(wiStr));
              } else {
                wDays.forEach(d2 => {
                  if (sched[pick.id][d2.id] !== "V" && sched[pick.id][d2.id] !== "O30") sched[pick.id][d2.id] = "O40";
                });
              }
              break;
            }
      ...
      Object.keys(lostRep).forEach(key => {
        ...
        for (const wi of wIdxs) {
          ...
          if (wDays.some(d => EMPLOYEES.filter(e => sched[e.id][d.id] === "O30").length >= 3)) continue;
          wDays.forEach(d => {
            if (sched[empId][d.id] !== "V") sched[empId][d.id] = "O30";
          });
          break;
        }
      });
      // Restore edge week alternation for all employees
      [0, 1, 16, 17].forEach(wi => {
        const wDays = wmFix[wi];
        if (!wDays) return;
        const shouldBeALate = wi % 2 === 0;
        wDays.forEach(day => {
          EMPLOYEES.forEach(emp => {
            if (sched[emp.id][day.id] === "V") return;
            const isLateGroup = shouldBeALate ? emp.group === "A" : emp.group === "B";
            sched[emp.id][day.id] = isLateGroup ? "O42" : "O40";
          });
        });
      });
      ensureMinSixWeeksAllFinal(sched);
    };
    finalizeReconciliation(finalStrictAudit.schedule);
  }
  const refreshedStrictValidation = validateStrictWeeklyRules({
    employees: EMPLOYEES,
    days,
    schedule: finalStrictAudit.schedule
  });
  finalStrictAudit.violations = refreshedStrictValidation.violations;
  finalStrictAudit.summary = refreshedStrictValidation.summary;
  return {
    schedule: finalStrictAudit.schedule,
    days,
    strictAudit: {
      violations: finalStrictAudit.violations,
      corrections: finalStrictAudit.corrections,
      summary: finalStrictAudit.summary
    }
  };
};
const LoginForm = ({
...
const CoverageLegend = () => /*#__PURE__*/React.createElement("div", {
  className: "rounded-xl border border-gray-200 bg-white p-4 shadow-sm"
}, /*#__PURE__*/React.createElement("div", {
  className: "flex items-start justify-between gap-3"
}, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("h3", {
  className: "text-sm font-bold text-gray-800"
}, "Leyenda de cobertura"), /*#__PURE__*/React.createElement("p", {
  className: "text-xs text-gray-500"
}, "Las bandas reflejan la hora final del turno visible en cada d\xEDa.")), /*#__PURE__*/React.createElement("span", {
  className: "rounded-full bg-gray-100 px-2.5 py-1 text-[11px] font-semibold text-gray-600"
}, "14h \xB7 17h \xB7 18h")), /*#__PURE__*/React.createElement("div", {
  className: "mt-4 grid grid-cols-1 gap-2 sm:grid-cols-3"
}, COVERAGE_BAND_ORDER.map(bandKey => {
  const meta = COVERAGE_BANDS[bandKey];
  return /*#__PURE__*/React.createElement("div", {
    key: bandKey,
    className: `rounded-lg border ${meta.border} ${meta.soft} px-3 py-2`
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, bandKey === "14" ? "Intensiva, viernes 42h y teletrabajo 30h." : bandKey === "17" ? "Cobertura operativa estándar." : "Cobertura extendida de tarde."));
})));
...
const App = () => {
  ...
  const weeklyCoverage = useMemo(() => buildWeeklyCoverageViewModel({
    days: coverageDays,
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "bg-brand-blue/5 p-3 rounded-2xl"
  }, /*#__PURE__*/React.createElement("img", {
    src: "logo.png",
    alt: "Logo",
  ...
  }, "Horarios Sistemas"), /*#__PURE__*/React.createElement("p", {
    className: "text-gray-500 text-sm font-medium"
  }, "Gesti\xF3n Operativa 2026"))), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-2 self-end"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: handleLogout,
    className: "bg-gray-50 hover:bg-gray-100 text-gray-600 px-4 py-2.5 rounded-xl text-sm font-bold transition-all border border-gray-200"
  }, "Cerrar Sesi\xF3n"))), /*#__PURE__*/React.createElement("div", {
    className: "mt-8 grid grid-cols-1 lg:grid-cols-12 gap-4"
  }, /*#__PURE__*/React.createElement("div", {
    className: "lg:col-span-4 bg-gray-50/50 border border-gray-100 rounded-2xl p-2 flex gap-2 shadow-sm"
  }, /*#__PURE__*/React.createElement("select", {
    className: "flex-1 bg-white text-gray-900 border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-semibold focus:ring-2 focus:ring-brand-blue/10 focus:border-brand-blue outline-none transition-all",
    value: selectedEmp,
    onChange: e => setSelectedEmp(e.target.value)
  }, /*#__PURE__*/React.createElement("option", {
    value: "all"
  }, "Integrantes: Todos"), EMPLOYEES.map(e => /*#__PURE__*/React.createElement("option", {
    key: e.id,
    value: e.id
    ...
    placeholder: "Buscar...",
    className: "flex-1 bg-white text-gray-900 border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-semibold focus:ring-2 focus:ring-brand-blue/10 focus:border-brand-blue outline-none transition-all min-w-0"
  })), /*#__PURE__*/React.createElement("div", {
    ...
    className: `flex-1 rounded-xl px-2 py-2.5 text-sm font-black transition-all ${viewMode === option.key ? "bg-fuchsia-600 text-white shadow-lg shadow-fuchsia-200" : "text-gray-500 hover:bg-white hover:text-gray-700"}`
  }, option.label))), /*#__PURE__*/React.createElement("div", {
    className: "lg:col-span-4 flex gap-2"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: () => setOListOpen(true),
    className: "flex-1 bg-white hover:bg-gray-50 text-gray-700 px-4 py-2.5 rounded-2xl text-sm font-bold border border-gray-200 shadow-sm transition-all"
  }, "Forzadas"), /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setVacationSectionOpen(prev => !prev);
      if (!vacationSectionOpen) setExportModalOpen(false);
    },
    className: `flex-1 px-4 py-2.5 rounded-2xl text-sm font-bold shadow-md transition-all ${vacationSectionOpen ? "bg-teal-700 text-white" : "bg-teal-600 hover:bg-teal-700 text-white"}`
  }, "Vacaciones"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setExportModalOpen(true),
    className: "flex-1 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-2xl text-sm font-black shadow-lg shadow-emerald-100 transition-all flex items-center justify-center gap-2"
  }, /*#__PURE__*/React.createElement("svg", {
    xmlns: "http://www.w3.org/2000/svg",
    width: "16",
    height: "16",
    ...
    className: `flex-1 min-w-[50px] py-2 rounded-lg text-xs font-bold transition-all border ${year === y ? "bg-brand-blue text-white border-brand-blue shadow-md shadow-brand-blue/20" : "bg-white text-gray-600 border-gray-200 hover:border-brand-blue/30 hover:bg-gray-50"}`
  ...
  })()), viewMode === "coverage" && /*#__PURE__*/React.createElement("div", {
    className: "mb-6 space-y-6"
  }, /*#__PURE__*/React.createElement("div", {
    className: "grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,0.9fr)]"
  }, /*#__PURE__*/React.createElement("div", {
    className: "rounded-xl border border-gray-200 bg-white p-4 shadow-sm"
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, "Vista semanal de cobertura"), /*#__PURE__*/React.createElement("p", {
    className: "text-sm text-gray-500"
  }, "Resume la cobertura diaria por fin de jornada para detectar r\xE1pidamente tramos hasta las 14h, 17h y 18h.")), /*#__PURE__*/React.createElement("div", {
  ...
  }, coverageSummary.weeks)), /*#__PURE__*/React.createElement("div", {
    className: "rounded-lg border border-gray-200 bg-gray-50 px-3 py-2"
  }, /*#__PURE__*/React.createElement("div", {
    className: "text-gray-500"
  }, "D\xEDas"), /*#__PURE__*/React.createElement("div", {
    className: "text-lg font-bold text-gray-800"
  }, coverageSummary.days)), /*#__PURE__*/React.createElement("div", {
    className: "rounded-lg border border-gray-200 bg-gray-50 px-3 py-2"
  }, /*#__PURE__*/React.createElement("div", {
    className: "text-gray-500"
  }, "Integrantes"), /*#__PURE__*/React.createElement("div", {
    className: "text-lg font-bold text-gray-800"
  }, coverageSummary.employees)), /*#__PURE__*/React.createElement("div", {
    className: "rounded-lg border border-gray-200 bg-gray-50 px-3 py-2"
  }, /*#__PURE__*/React.createElement("div", {
    className: "text-gray-500"
  ...
  }, "Modo visual"), /*#__PURE__*/React.createElement("select", {
  ...
  }, COVERAGE_VISUALIZATION_OPTIONS.map(option => /*#__PURE__*/React.createElement("option", {
    key: option.value,
    value: option.value
  }, option.label)))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    htmlFor: "coverage-start-date",
    className: "mb-1 block text-xs font-semibold text-gray-500"
    ...
    value: "all"
  }, "Todos los tramos"), COVERAGE_BAND_ORDER.map(bandKey => /*#__PURE__*/React.createElement("option", {
    key: bandKey,
    value: bandKey
  }, COVERAGE_BANDS[bandKey].label))))), /*#__PURE__*/React.createElement("div", {
  ...
  }, "Limpiar filtros"), /*#__PURE__*/React.createElement("span", {
    className: "rounded-full bg-gray-100 px-2.5 py-1 text-[11px] text-gray-600"
  }, "Rango: ", normalizedCoverageDates.startDate || "—", " \u2192 ", normalizedCoverageDates.endDate || "—"), /*#__PURE__*/React.createElement("span", {
    className: "rounded-full bg-violet-50 px-2.5 py-1 text-[11px] text-violet-700"
  }, "Promedio hasta 18h: ", coverageSummary.avgActiveCounts["18"].toFixed(1)), /*#__PURE__*/React.createElement("span", {
    className: "rounded-full bg-sky-50 px-2.5 py-1 text-[11px] text-sky-700"
  }, "Promedio hasta 17h: ", coverageSummary.avgActiveCounts["17"].toFixed(1)), /*#__PURE__*/React.createElement("span", {
    className: "rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] text-emerald-700"
  }, "Promedio hasta 14h: ", coverageSummary.avgActiveCounts["14"].toFixed(1)))), /*#__PURE__*/React.createElement(CoverageLegend, null)), weeklyCoverage.length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "rounded-xl border border-dashed border-gray-300 bg-white p-10 text-center text-sm text-gray-500 shadow-sm"
  }, "No hay datos de cobertura para los filtros seleccionados.") : /*#__PURE__*/React.createElement("div", {
    className: "space-y-4"
  }, weeklyCoverage.map(week => /*#__PURE__*/React.createElement("div", {
    key: week.weekIndex,
  ...
  }, week.weekLabel, " \xB7 ", week.periodLabel), /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-gray-500"
  }, week.month)), /*#__PURE__*/React.createElement("div", {
    className: "flex flex-wrap gap-2"
  }, COVERAGE_BAND_ORDER.map(bandKey => /*#__PURE__*/React.createElement("span", {
    ...
    className: `h-2.5 w-2.5 rounded-full ${COVERAGE_BANDS[bandKey].color}`
  }), COVERAGE_BANDS[bandKey].short, ": ", week.totals[bandKey]))))), coverageVisualization === "stacked" && /*#__PURE__*/React.createElement("div", {
    className: "space-y-3 p-4"
  }, week.days.map(dayEntry => /*#__PURE__*/React.createElement("div", {
    key: dayEntry.dayId,
    ...
    className: "text-sm font-semibold text-gray-800"
  }, dayEntry.dayLabel), /*#__PURE__*/React.createElement("div", {
  ...
  }, COVERAGE_BANDS[bandKey].short, ": ", dayEntry.counts[bandKey])))), /*#__PURE__*/React.createElement("div", {
    className: "space-y-3"
  }, /*#__PURE__*/React.createElement("div", {
    className: "relative h-24 rounded-xl border border-gray-200 bg-slate-50 px-3 py-2"
  }, /*#__PURE__*/React.createElement("div", {
    className: "absolute inset-x-3 top-1 flex justify-between text-[10px] font-medium text-gray-400"
  }, /*#__PURE__*/React.createElement("span", null, "08:00"), /*#__PURE__*/React.createElement("span", null, "14:00"), /*#__PURE__*/React.createElement("span", null, "17:00"), /*#__PURE__*/React.createElement("span", null, "18:00")), /*#__PURE__*/React.createElement("div", {
    className: "absolute inset-x-3 bottom-3 top-7 space-y-2"
  }, ["18", "17", "14"].map(bandKey => {
    const meta = COVERAGE_BANDS[bandKey];
    return /*#__PURE__*/React.createElement("div", {
      key: `${dayEntry.dayId}-bar-${bandKey}`,
      className: "flex items-center gap-2"
    }, /*#__PURE__*/React.createElement("span", {
      className: `w-10 text-[11px] font-semibold ${meta.text}`
    }, meta.short), /*#__PURE__*/React.createElement("div", {
      className: "relative h-4 flex-1 rounded-full bg-gray-100"
    }, /*#__PURE__*/React.createElement("div", {
      className: `h-4 rounded-full ${meta.color} opacity-80`,
      style: {
        width: `${dayEntry.counts[bandKey] > 0 ? COVERAGE_TIMELINE_WIDTH[bandKey] : 0}%`
      }
    })), /*#__PURE__*/React.createElement("span", {
      className: "w-7 text-right text-[11px] font-semibold text-gray-600"
    }, dayEntry.counts[bandKey]));
  }))), /*#__PURE__*/React.createElement("div", {
    className: "grid grid-cols-3 gap-2"
  }, dayEntry.timeline.map(segment => /*#__PURE__*/React.createElement("div", {
    key: `${dayEntry.dayId}-${segment.key}`,
  ...
  }, segment.label), /*#__PURE__*/React.createElement("div", {
    className: "mt-1 text-lg font-bold text-gray-800"
  }, segment.count), /*#__PURE__*/React.createElement("div", {
    className: "text-[10px] text-gray-500"
  }, "activos"))))), /*#__PURE__*/React.createElement("div", {
    className: "space-y-2"
  }, COVERAGE_BAND_ORDER.map(bandKey => /*#__PURE__*/React.createElement("div", {
    key: `${dayEntry.dayId}-names-${bandKey}`,
    className: `rounded-lg border ${COVERAGE_BANDS[bandKey].border} p-2`
  }, /*#__PURE__*/React.createElement("div", {
    className: `text-[11px] font-bold ${COVERAGE_BANDS[bandKey].text}`
  }, COVERAGE_BANDS[bandKey].label), /*#__PURE__*/React.createElement("div", {
    className: "mt-1 flex flex-wrap gap-1"
  }, dayEntry.assignmentsByBand[bandKey].length === 0 ? /*#__PURE__*/React.createElement("span", {
    className: "text-[11px] text-gray-400"
  }, "Sin asignaciones") : dayEntry.assignmentsByBand[bandKey].map(assignment => /*#__PURE__*/React.createElement("span", {
    ...
    className: "text-[10px] text-gray-500"
  }, assignment.isForcedOffice ? "OF" : assignment.isInOffice ? "OFI" : "TT")))))))))), coverageVisualization === "timeline" && /*#__PURE__*/React.createElement("div", {
    className: "space-y-3 p-4"
  }, week.days.map(dayEntry => /*#__PURE__*/React.createElement("div", {
    key: dayEntry.dayId,
    ...
    className: "text-xs text-gray-500"
  }, dayEntry.totalAssignments, " personas visibles")), /*#__PURE__*/React.createElement("div", {
    className: "grid grid-cols-1 gap-2 md:grid-cols-3"
  }, dayEntry.timeline.map(segment => /*#__PURE__*/React.createElement("div", {
    key: `${dayEntry.dayId}-timeline-${segment.key}`,
    className: "rounded-lg border border-gray-200 bg-gray-50 p-3"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between"
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-xs font-semibold text-gray-500"
  }, segment.label), /*#__PURE__*/React.createElement("span", {
    className: "text-lg font-bold text-gray-800"
  }, segment.count)), /*#__PURE__*/React.createElement("div", {
    className: "mt-3 flex h-2 overflow-hidden rounded-full bg-gray-200"
  }, segment.bands.map(bandKey => /*#__PURE__*/React.createElement("div", {
  ...
  }))))))))), coverageVisualization === "heatmap" && /*#__PURE__*/React.createElement("div", {
  ...
  }, "D\xEDa"), ["14", "17", "18"].map(bandKey => /*#__PURE__*/React.createElement("th", {
    key: `${week.weekIndex}-heat-head-${bandKey}`,
    className: "border-b border-gray-200 px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-gray-500"
  }, "Activos ", COVERAGE_BANDS[bandKey].short)))), /*#__PURE__*/React.createElement("tbody", null, week.days.map(dayEntry => /*#__PURE__*/React.createElement("tr", {
    key: `${dayEntry.dayId}-heat-row`
  }, /*#__PURE__*/React.createElement("td", {
    className: "border-b border-gray-100 px-3 py-2 font-medium text-gray-700"
  }, dayEntry.dayLabel), ["14", "17", "18"].map(bandKey => {
    const intensity = dayEntry.maxCount > 0 ? dayEntry.activeCounts[bandKey] / dayEntry.maxCount : 0;
    return /*#__PURE__*/React.createElement("td", {
      key: `${dayEntry.dayId}-heat-${bandKey}`,
      className: "border-b border-gray-100 px-3 py-2"
    }, /*#__PURE__*/React.createElement("div", {
      className: `rounded-lg px-3 py-3 text-sm font-bold ${COVERAGE_BANDS[bandKey].text}`,
      style: {
        backgroundColor: bandKey === "14" ? `rgba(16, 185, 129, ${0.12 + intensity * 0.45})` : bandKey === "17" ? `rgba(14, 165, 233, ${0.12 + intensity * 0.45})` : `rgba(139, 92, 246, ${0.12 + intensity * 0.45})`
      }
    }, dayEntry.activeCounts[bandKey]));
  })))))), coverageVisualization === "table" && /*#__PURE__*/React.createElement("div", {
    className: "overflow-x-auto p-4"
  }, /*#__PURE__*/React.createElement("table", {
    className: "w-full min-w-[820px] border-collapse text-sm"
  }, /*#__PURE__*/React.createElement("thead", null, /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("th", {
    className: "border-b border-gray-200 px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-gray-500"
  }, "D\xEDa"), COVERAGE_BAND_ORDER.map(bandKey => /*#__PURE__*/React.createElement("th", {
    key: `${week.weekIndex}-table-${bandKey}`,
    className: "border-b border-gray-200 px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-gray-500"
  }, COVERAGE_BANDS[bandKey].label)))), /*#__PURE__*/React.createElement("tbody", null, week.days.map(dayEntry => /*#__PURE__*/React.createElement("tr", {
    key: `${dayEntry.dayId}-table-row`
  }, /*#__PURE__*/React.createElement("td", {
    className: "border-b border-gray-100 px-3 py-2 font-medium text-gray-700"
  }, dayEntry.dayLabel), COVERAGE_BAND_ORDER.map(bandKey => /*#__PURE__*/React.createElement("td", {
    key: `${dayEntry.dayId}-table-cell-${bandKey}`,
    className: "border-b border-gray-100 px-3 py-2 align-top"
  }, /*#__PURE__*/React.createElement("div", {
    className: `min-h-[68px] rounded-lg border ${COVERAGE_BANDS[bandKey].border} ${COVERAGE_BANDS[bandKey].soft} p-2`
  }, /*#__PURE__*/React.createElement("div", {
    className: `text-xs font-bold ${COVERAGE_BANDS[bandKey].text}`
  }, dayEntry.counts[bandKey], " personas"), /*#__PURE__*/React.createElement("div", {
    className: "mt-2 space-y-1"
  }, dayEntry.assignmentsByBand[bandKey].length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "text-[11px] text-gray-400"
  }, "Sin cobertura") : dayEntry.assignmentsByBand[bandKey].map(assignment => /*#__PURE__*/React.createElement("div", {
    key: `${dayEntry.dayId}-${bandKey}-table-name-${assignment.id}`,
    className: "text-[11px] text-gray-700"
  }, assignment.name, " \xB7 ", assignment.role))))))))))), coverageVisualization === "calendar" && /*#__PURE__*/React.createElement("div", {
    className: "grid grid-cols-1 gap-4 p-4 xl:grid-cols-5"
  }, week.days.map(dayEntry => /*#__PURE__*/React.createElement("div", {
    key: `${dayEntry.dayId}-calendar`,
    className: "rounded-xl border border-gray-200 bg-white p-3 shadow-sm"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    className: "text-xs uppercase tracking-wide text-gray-400"
  }, dayEntry.weekdayName), /*#__PURE__*/React.createElement("div", {
    className: "text-lg font-bold text-gray-800"
  }, dayEntry.dateNumber)), /*#__PURE__*/React.createElement("div", {
    className: "rounded-full bg-gray-100 px-2 py-1 text-[11px] text-gray-600"
  }, dayEntry.totalAssignments, " visibles")), /*#__PURE__*/React.createElement("div", {
    className: "mt-3 space-y-2"
  }, COVERAGE_BAND_ORDER.map(bandKey => /*#__PURE__*/React.createElement("div", {
    key: `${dayEntry.dayId}-calendar-band-${bandKey}`,
    className: `rounded-lg border ${COVERAGE_BANDS[bandKey].border} p-2`
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between"
  }, /*#__PURE__*/React.createElement("span", {
    className: `text-xs font-bold ${COVERAGE_BANDS[bandKey].text}`
  }, COVERAGE_BANDS[bandKey].short), /*#__PURE__*/React.createElement("span", {
    className: "text-xs text-gray-500"
  }, dayEntry.counts[bandKey])), /*#__PURE__*/React.createElement("div", {
    className: "mt-2 flex flex-wrap gap-1"
  }, dayEntry.assignmentsByBand[bandKey].length === 0 ? /*#__PURE__*/React.createElement("span", {
    className: "text-[11px] text-gray-400"
  }, "\u2014") : dayEntry.assignmentsByBand[bandKey].map(assignment => /*#__PURE__*/React.createElement("span", {
  ...
  }, assignment.name))))))))), coverageVisualization === "gantt" && /*#__PURE__*/React.createElement("div", {
    className: "space-y-3 p-4"
  }, week.days.map(dayEntry => /*#__PURE__*/React.createElement("div", {
    key: `${dayEntry.dayId}-gantt`,
    className: "grid grid-cols-1 gap-3 rounded-lg border border-gray-100 p-3 lg:grid-cols-[220px_minmax(0,1fr)]"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    className: "text-sm font-semibold text-gray-800"
  }, dayEntry.dayLabel), /*#__PURE__*/React.createElement("div", {
    className: "mt-1 text-xs text-gray-500"
  }, "Cobertura activa 18h: ", dayEntry.activeCounts["18"], " \xB7 17h: ", dayEntry.activeCounts["17"], " \xB7 14h: ", dayEntry.activeCounts["14"])), /*#__PURE__*/React.createElement("div", {
    className: "rounded-xl border border-gray-200 bg-slate-50 px-3 py-3"
  }, /*#__PURE__*/React.createElement("div", {
    className: "mb-3 flex justify-between text-[10px] font-medium text-gray-400"
  }, /*#__PURE__*/React.createElement("span", null, "08:00"), /*#__PURE__*/React.createElement("span", null, "14:00"), /*#__PURE__*/React.createElement("span", null, "17:00"), /*#__PURE__*/React.createElement("span", null, "18:00")), /*#__PURE__*/React.createElement("div", {
    className: "space-y-2"
  }, dayEntry.ganttBars.length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "text-sm text-gray-400"
  }, "Sin asignaciones para este filtro.") : dayEntry.ganttBars.map(bar => /*#__PURE__*/React.createElement("div", {
    key: `${dayEntry.dayId}-gantt-bar-${bar.key}`,
    className: "flex items-center gap-2"
  }, /*#__PURE__*/React.createElement("div", {
    className: `w-10 text-xs font-semibold ${bar.meta.text}`
  }, bar.meta.short), /*#__PURE__*/React.createElement("div", {
    className: "h-6 flex-1 rounded-full bg-gray-100"
  }, /*#__PURE__*/React.createElement("div", {
    className: `flex h-6 items-center justify-end rounded-full pr-2 text-xs font-bold text-white ${bar.meta.color}`,
    style: {
      width: `${bar.width}%`
    }
  }, bar.count))))))))))))), viewMode === "matrix" ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    ...
    }, hasAlert && /*#__PURE__*/React.createElement("button", {
      type: "button",
